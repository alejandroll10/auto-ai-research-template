...
| **BACK-TO-IDEA** | Return to Stage 1 with the triager report as input to the idea-reviewer. **Do NOT increment `problem_attempt`** (the problem is unchanged — only the idea is being replaced). Set `current_stage: "stage_1"`. Before re-running idea-reviewer, apply `docs/stage_1.md` step 2 re-entry logic: prefer a pre-screened runner-up from `pipeline_state.json:stage1_candidates` (entry with `eliminated: false AND winner: false`) over regenerating. Skip if Stage 5 has begun (paper exists) — use HONEST-NULL instead. |
